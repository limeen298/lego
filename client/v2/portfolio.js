...
const fetchDeals = async (page = 1, size = 6) => {
  try {
    const response = await fetch(
      `https://lego-server-3ua1gwde9-limeen298s-projects.vercel.app/api/deals?page=${page}&size=${size}`
    );
...
const fetchSales = async (id) => {
  try {
    const response = await fetch(
      `https://lego-server-3ua1gwde9-limeen298s-projects.vercel.app/api/sales?id=${id}`
    );
    const body = await response.json();
